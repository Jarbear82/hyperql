...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_hyperql", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_hyperql": ["*.pyi", "py.typed"],
        "tree_sitter_hyperql.queries": ["*.scm"],
    },
    ext_package="tree_sitter_hyperql",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_hyperql/binding.c",
                "src/parser.c",
            ],
            define_macros=[
