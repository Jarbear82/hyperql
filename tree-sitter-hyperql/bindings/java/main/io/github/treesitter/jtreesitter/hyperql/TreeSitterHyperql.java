package io.github.treesitter.jtreesitter.hyperql;

import java.lang.foreign.*;

public final class TreeSitterHyperql {
    ...
    private static final TreeSitterHyperql INSTANCE = new TreeSitterHyperql();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterHyperql() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_hyperql");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-hyperql");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-hyperql");
